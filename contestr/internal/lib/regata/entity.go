...
type Table map[int]map[string]int
...
func (t *Tour) ParticipantScore(participant Participant) map[string]int {
	group := t.Groups[participant]
	score := 0
	participantResults := t.Results[participant]
	result := make(map[string]int, len(t.Problems))
...
func (t *Tour) Export() Table {
	...
	for _, group := range t.Groups {
		for _, participant := range group {
			result[int(participant)] = t.ParticipantScore(participant)
		}
	}
